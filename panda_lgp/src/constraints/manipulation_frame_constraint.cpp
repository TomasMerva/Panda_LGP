...
void 
Constraint::Test(unsigned m, double *result, unsigned n, const double* x, double* grad, void* f_data)
{
    ...
    if (grad)
    {
        ...
        {
            grad[i] = J(0,i);
            // grad[i+g_data->num_phase_variables] = J(1,i);
            // grad[i+2*g_data->num_phase_variables] = J(2,i);
        }
        grad[7] = -1;
        // grad[7+g_data->num_phase_variables] = -1;
        // grad[7+2*g_data->num_phase_variables] = -1;
    }
}